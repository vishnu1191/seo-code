  ...
  return (
    <section id="process" className="relative py-20 lg:py-32 overflow-hidden">
      ...
      <motion.div 
        ...
        className="absolute top-1/2 right-1/4 w-[600px] h-[600px] bg-[#4B147F]/15 rounded-full blur-[170px] pointer-events-none" 
      />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="relative mb-12 hidden lg:block">
          ...
          <motion.div 
            className="absolute top-7 left-12 h-[2px] bg-gradient-to-r from-[#8B3DFF] to-[#B15CFF] shadow-[0_0_12px_#8B3DFF]" 
            animate={{ width: `${(activeStep / (PROCESS_STEPS.length - 1)) * 88}%` }}
            ...
            {PROCESS_STEPS.map((step, idx) => {
              ...
              return (
                ...
                >
                  {/* Glowing Node Button */}
                  <motion.div
                    animate={isSelected ? { scale: [1, 1.08, 1.05] } : { scale: 1 }}
                    ...
                    className={`w-14 h-14 rounded-2xl flex items-center justify-center font-['Space_Grotesk'] font-bold text-sm transition-all duration-300 mb-6 ${
                      isSelected
                        ? 'bg-gradient-to-br from-[#8B3DFF] to-[#4B147F] text-white shadow-[0_0_25px_rgba(139,61,255,0.6)] border border-[#D7BFFF]/50'
        ...
        <motion.div 
          layout
          className="rounded-3xl bg-[#0A0810]/80 backdrop-blur-xl border border-white/5 p-8 sm:p-10 shadow-[0_20px_50px_-15px_rgba(75,20,127,0.3)]"
        >
                ...
                <div className="space-y-3">
                  {PROCESS_STEPS[activeStep].deliverables.map((item, idx) => (
                    <motion.div 
                      ...
                      className="flex items-center gap-3 text-xs sm:text-sm text-[#F7F5FA]"
                    >
